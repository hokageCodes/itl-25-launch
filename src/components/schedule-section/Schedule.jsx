...
const ScheduleTimeline = () => {
  ...
  const schedule = {
    Friday: [
      ...
      {
        title:
          "Enhancing Integration and Professional Development for Internationally Trained Lawyers: Perspectives from Regulators and the Judiciary (Panel Session)",
          ...
          {schedule[activeDay].map((event, index) => (
            ...
            >
              ...
              <div className="md:flex-grow">
                <h4 className="text-xl md:text-2xl font-bold mb-2 text-wine">
                  {event.title}
                </h4>
                ...
                {event.details && (
                  <button
                    ...
                    className="text-wine underline mt-1 text-sm"
                  >
                    See Details
                  </button>
                )}
...
{isModalOpen && selectedEvent && (
  <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
    <div className="bg-white p-6 rounded-lg max-w-lg w-full max-h-[80vh] overflow-y-auto">
      ...
      <button
        onClick={closeModal}
        className="mt-4 px-4 py-2 bg-wine text-white rounded-md"
      >
        Close
      </button>
    </div>
